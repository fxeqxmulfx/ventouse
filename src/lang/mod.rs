...
//! locality (P5).
//!
//! Ships the Python (`python`), Rust (`rust`), C++ (`cpp`) and TypeScript/JavaScript (`ts`, via
//! `oxc`, incl. JSX/React) frontends. The raw IR + the `Frontend` trait live in `core::raw`
//! (re-exported here for convenience); this module only holds the per-language frontends. All lower
//! to the same `core::scopelang::Action`s and share the entire core.
